...
class SvXData(bpy.types.Node, SverchCustomTreeNode):
    bl_idname = 'SvXData'
    bl_label = 'LB Deconstruct Data'
    sv_icon = 'LB_XDATA'
    ...
    def sv_init(self, context):
        ...
        output_node.tooltip = 'The header of the DataCollection (containing metadata).'
        output_node = self.outputs.new('SvLBSocket', 'values')
        output_node.tooltip = 'The numerical values of the DataCollection.'

    def draw_buttons(self, context, layout):
        op = layout.operator('node.sv_lb_socket_name', text='', icon='QUESTION', emboss=False).tooltip = 'Deconstruct a Ladybug DataCollection into a header and values. -'

    def process(self):
        if not any(socket.is_linked for socket in self.outputs):
            return

        self.sv_output_names = ['header', 'values']
...
def register():
    bpy.utils.register_class(SvXData)

def unregister():
    bpy.utils.unregister_class(SvXData)
