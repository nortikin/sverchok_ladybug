...
class SvDataDT(bpy.types.Node, SverchCustomTreeNode):
    bl_idname = 'SvDataDT'
    bl_label = 'LB Data DateTimes'
    sv_icon = 'LB_DATADT'
    sv__data: StringProperty(
        name='_data',
        update=updateNode,
        description='An hourly, daily, or monthly collection from which hours, days, or months of the year will be retrieved.')

    def sv_init(self, context):
        self.width *= 1.3
        input_node = self.inputs.new('SvLBSocket', '_data')
        input_node.prop_name = 'sv__data'
        input_node.tooltip = 'An hourly, daily, or monthly collection from which hours, days, or months of the year will be retrieved.'
        output_node = self.outputs.new('SvLBSocket', 'hoys')
        output_node.tooltip = 'Numbers for the, hours, days or months of the year in the data collection.'

    def draw_buttons(self, context, layout):
        op = layout.operator('node.sv_lb_socket_name', text='', icon='QUESTION', emboss=False).tooltip = 'Get the hours, days, or months of the year associated with the values of a data collection. -'

    def process(self):
        if not any(socket.is_linked for socket in self.outputs):
            return

        self.sv_output_names = ['hoys']
...
def register():
    bpy.utils.register_class(SvDataDT)

def unregister():
    bpy.utils.unregister_class(SvDataDT)
