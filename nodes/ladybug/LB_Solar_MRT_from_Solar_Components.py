...
class SvComponentSolarMRT(bpy.types.Node, SverchCustomTreeNode):
    bl_idname = 'SvComponentSolarMRT'
    bl_label = 'LB Solar MRT from Solar Components'
    sv_icon = 'LB_COMPONENTSOLARMRT'
    ...
    sv__dir_horiz_rad: StringProperty(
        name='_dir_horiz_rad',
        update=updateNode,
        description='Hourly Data Collection with the direct horizontal solar irradiance in W/m2.')
    ...
    sv_fract_body_exp_: StringProperty(
        ...
        update=updateNode,
        description='A single number between 0 and 1 or a data collection representing the fraction of the body exposed to direct sunlight. Note that this does not include the body’s self-shading; only the shading from surroundings. Default is 1 for a person standing in an open area.')
    ...
    def sv_init(self, context):
        ...
        input_node.tooltip = 'A Ladybug Location object.'
        ...
        input_node = self.inputs.new('SvLBSocket', '_dir_horiz_rad')
        input_node.prop_name = 'sv__dir_horiz_rad'
        input_node.tooltip = 'Hourly Data Collection with the direct horizontal solar irradiance in W/m2.'
        ...
        input_node.tooltip = 'A single number between 0 and 1 or a data collection representing the fraction of the body exposed to direct sunlight. Note that this does not include the body’s self-shading; only the shading from surroundings. Default is 1 for a person standing in an open area.'
    ...
    def draw_buttons(self, context, layout):
        op = layout.operator('node.sv_lb_socket_name', text='', icon='QUESTION', emboss=False).tooltip = 'Calculate Mean Radiant Temperature (MRT) as a result of shortwave solar using horizontal solar components (direct horizontal and diffuse horizontal solar). - This component uses the SolarCal model of ASHRAE-55 to estimate the effects of shortwave solar and a simple sky exposure method to determine longwave radiant exchange. -'
    ...
    def process(self):
        ...
        self.sv_input_names = ['_location', '_longwave_mrt', '_dir_horiz_rad', '_diff_horiz_rad', 'fract_body_exp_', '_ground_ref_', '_solar_body_par_', '_run']
    ...
    def process_ladybug(self, _location, _longwave_mrt, _dir_horiz_rad, _diff_horiz_rad, fract_body_exp_, _ground_ref_, _solar_body_par_, _run):
...
def register():
    bpy.utils.register_class(SvComponentSolarMRT)

def unregister():
    bpy.utils.unregister_class(SvComponentSolarMRT)
