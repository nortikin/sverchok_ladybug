...
class SvOutdoorSolarMRT(bpy.types.Node, SverchCustomTreeNode):
    bl_idname = 'SvOutdoorSolarMRT'
    bl_label = 'LB Outdoor Solar MRT'
    sv_icon = 'LB_OUTDOORSOLARMRT'
    sv__location: StringProperty(
        name='_location',
        update=updateNode,
        description='A Ladybug Location object, used to determine the altitude and azimuth of the sun.')
    sv__surface_temp: StringProperty(
        name='_surface_temp',
        update=updateNode,
        description='A single number or an hourly data collection with the temperature of surfaces around the person in degrees C. This includes the ground and any other surfaces blocking the view to the sky. Typically, outdoor dry bulb temperature is used when such surface temperatures are unknown.')
    ...
    sv__horiz_infrared: StringProperty(
        name='_horiz_infrared',
        update=updateNode,
        description='Hourly Data Collection with the horizontal infrared radiation intensity from the sky in W/m2.')
    ...
    sv__ground_ref_: StringProperty(
        name='_ground_ref_',
        update=updateNode,
        description='A single number between 0 and 1 or a data collection that represents the reflectance of the floor. Default is for 0.25 which is characteristic of outdoor grass or dry bare soil.')
    ...
    def sv_init(self, context):
        ...
        input_node = self.inputs.new('SvLBSocket', '_surface_temp')
        input_node.prop_name = 'sv__surface_temp'
        input_node.tooltip = 'A single number or an hourly data collection with the temperature of surfaces around the person in degrees C. This includes the ground and any other surfaces blocking the view to the sky. Typically, outdoor dry bulb temperature is used when such surface temperatures are unknown.'
        ...
        input_node = self.inputs.new('SvLBSocket', '_horiz_infrared')
        input_node.prop_name = 'sv__horiz_infrared'
        input_node.tooltip = 'Hourly Data Collection with the horizontal infrared radiation intensity from the sky in W/m2.'
        ...
        input_node.tooltip = 'A single number between 0 and 1 or a data collection that represents the reflectance of the floor. Default is for 0.25 which is characteristic of outdoor grass or dry bare soil.'
        ...
        output_node = self.outputs.new('SvLBSocket', 'short_erf')
        output_node.tooltip = 'Data collection of shortwave effective radiant field (ERF) in W/m2.'
        output_node = self.outputs.new('SvLBSocket', 'long_erf')
        output_node.tooltip = 'Data collection of longwave effective radiant field (ERF) in W/m2.'
        output_node = self.outputs.new('SvLBSocket', 'short_dmrt')
        output_node.tooltip = 'Data collection of shortwave mean radiant temperature delta in C.'
        output_node = self.outputs.new('SvLBSocket', 'long_dmrt')
        output_node.tooltip = 'Data collection of longwave mean radiant temperature delta in C.'
        output_node = self.outputs.new('SvLBSocket', 'mrt')
        output_node.tooltip = 'Data collection of mean radiant temperature in C.  This accounts for both the shortwave solar shining directly onto people as well as longwave radiant exchange with the sky.'
    ...
    def process(self):
        ...
        self.sv_output_names = ['short_erf', 'long_erf', 'short_dmrt', 'long_dmrt', 'mrt']
        for name in self.sv_output_names:
            setattr(self, '{}_out'.format(name), [])
        self.sv_input_names = ['_location', '_surface_temp', '_dir_norm_rad', '_diff_horiz_rad', '_horiz_infrared', 'fract_body_exp_', 'sky_exposure_', '_ground_ref_', '_solar_body_par_', '_run']
    ...
    def process_ladybug(self, _location, _surface_temp, _dir_norm_rad, _diff_horiz_rad, _horiz_infrared, fract_body_exp_, sky_exposure_, _ground_ref_, _solar_body_par_, _run):
...
def register():
    bpy.utils.register_class(SvOutdoorSolarMRT)

def unregister():
    bpy.utils.unregister_class(SvOutdoorSolarMRT)
