...
class SvDirectSunHours(bpy.types.Node, SverchCustomTreeNode):
    bl_idname = 'SvDirectSunHours'
    bl_label = 'LB Direct Sun Hours'
    sv_icon = 'LB_DIRECTSUNHOURS'
    sv__vectors: StringProperty(
        name='_vectors',
        update=updateNode,
        description='Sun vectors from the "LB SunPath" component, which will be used to determine the number of hours of direct sunlight received by the test _geometry.')
    ...
    sv_context_: StringProperty(
        name='context_',
        update=updateNode,
        description='Rhino Breps and/or Rhino Meshes representing context geometry that can block sunlight to the test _geometry.')
    ...
    def sv_init(self, context):
        ...
        input_node.tooltip = 'Sun vectors from the "LB SunPath" component, which will be used to determine the number of hours of direct sunlight received by the test _geometry.'
        ...
        input_node.tooltip = 'Rhino Breps and/or Rhino Meshes representing context geometry that can block sunlight to the test _geometry.'
        ...
        input_node.tooltip = 'Set to "True" to run the component and perform direct sun analysis.'
        output_node = self.outputs.new('SvLBSocket', 'points')
        output_node.tooltip = 'The grid of points on the test _geometry that are be used to perform the direct sun analysis.'
        output_node = self.outputs.new('SvLBSocket', 'results')
        output_node.tooltip = 'A list of numbers that aligns with the points. Each number indicates the number of hours of direct sunlight received by each of the points.  Note that is is the number of hours out of the total number of connected _vectors.'
        output_node = self.outputs.new('SvLBSocket', 'mesh')
        output_node.tooltip = 'A colored mesh of the test _geometry representing the hours of direct sunlight received by this input _geometry'
    ...
    def draw_buttons(self, context, layout):
        op = layout.operator('node.sv_lb_socket_name', text='', icon='QUESTION', emboss=False).tooltip = 'Calculate the number of hours of direct sunlight received by geometry using sun vectors obtained from the "LB SunPath" component. _ Such direct sun calculations can be used for shadow studies of outdoor enviroments or can be used to estimate glare potential from direct sun on the indoors. _ Note that this component uses the CAD environment\'s ray intersection methods, which can be fast for geometries with low complexity but does not scale well for complex geometries or many test points. For such complex studies, honeybee-radiance should be used. -'
    ...
    def process(self):
        ...
        self.sv_input_names = ['_vectors', '_timestep_', '_geometry', 'context_', '_grid_size', '_offset_dist_', 'legend_par_', '_cpu_count_', '_run']
        self.sv_input_types = ['Vector3d', 'int', 'GeometryBase', 'GeometryBase', 'double', 'double', 'System.Object', 'int', 'bool']
        self.sv_input_defaults = [None, None, None, None, None, None, None, None, None]
        self.sv_input_access = ['list', 'item', 'list', 'list', 'item', 'item', 'item', 'item', 'item']
        sv_inputs_nested = []
    ...
    def process_ladybug(self, _vectors, _timestep_, _geometry, context_, _grid_size, _offset_dist_, legend_par_, _cpu_count_, _run):
...
def register():
    bpy.utils.register_class(SvDirectSunHours)

def unregister():
    bpy.utils.unregister_class(SvDirectSunHours)
